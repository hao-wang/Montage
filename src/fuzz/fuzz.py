...
class Fuzzer:
    def __init__(self, proc_idx, conf):
        ...
        self._bug_dir = os.path.join(conf.bug_dir, 'proc.%d' % proc_idx)
        self._timeout = conf.timeout
        self._top_k = conf.top_k
        self._no_fuzz = conf.no_fuzz
        self._num_js = conf.num_js
    ...
    def fuzz(self):
        model = load_model(self._model_path, self._gpu)
...
def load_data(conf):
    data_path = os.path.join(conf.data_dir, 'data.p')
    seed_data_path = os.path.join(conf.data_dir, 'seed.p')
